...
| Fecha | Cambio |
|---|---|
| 2026-08-05 | Sesion TMS/jornadas + contratos (ver SPEC 0043). 15 archivos modificados, 6 areas: (1) Entrega desde composicion con serial rapido y cards de composicion en DELIVERY, (2) Contexto operativo automatico: parada por defecto, inferencia CUSTOMER vs WAREHOUSE desde delivery_point, simplificacion CUSTOMER_EMERGENCY→CUSTOMER, (3) Fix de seriales en ruta: orden de chequeos en select_load_serial, selectionContext dinamico LOAD_PLAN vs ROUTE_PICKUP, guard de isFetching en LoadSerialsDialog, (4) Fix de carta porte: migracion 050 (source_id VARCHAR(36)→255), _delivery_point_for_operation con fallback a operation.customer_id, (5) Transiciones SC para envases vacios: _cylinder_is_empty por content_kg, target_state condicional EN_CLIENTE_VACIO vs EN_CLIENTE_LLENO, transicion EN_RUTA→EN_CLIENTE_VACIO en catalogo + DB, (6) UI: colores de parada por progreso (STOP_STATUS_BORDER_COLORS, marcadores completed en mapa), modal Contexto de ruta 1800px/92vh, autoFit en LocationMap. Discusion de contratos: motor de exceso, venta vs alquiler B14/B50, facturacion diferida. |
