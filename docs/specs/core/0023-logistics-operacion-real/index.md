...
| Tema | Regla / hallazgo de Grab2 | Legacy documentado | Sistema nuevo | Estado comparado | Gap exacto | Owner natural | Spec / sub-spec sugerida |
|---|---|---|---|---|---|---|---|
| Camion como almacen movil | El camion funciona como un almacen movil real | Parcialmente implicito en legacy | Runtime retirado y rediseñado | Gap importante | La implementacion previa fue retirada; la reconstruccion vigente parte de `VehicleSession` y Stock como unica fuente de verdad. La alineacion posterior con operacion de calle queda en `0023E2`. | `logistics` | `SPEC 0023E1` VehicleSession + warehouse MOBILE / `SPEC 0023E2` Alineacion operativa |
...
> **Code Reality Check (2026-07-04, actualizado tras retiro runtime):** 0023E (almacén móvil) — la implementacion runtime fue retirada del producto por inconsistencia operativa y tecnica; `0023E1` define la reconstruccion vigente centrada en `VehicleSession`, `Warehouse(type=MOBILE)` y Stock como unica fuente de verdad. 0023F (agenda→carga) — **no existe el flujo**, es el gap operativo más crítico del dominio. 0023G (stock libre) — modelo de comprometido sólido, cero de stock extra. 0023H (escaneo) — infraestructura de scan sólida pero atada a movements, no a loads. 0023I (confirmación conductor) — `confirm_loads` existe pero sin identidad de conductor. Retorno a almacén: `vehicle_return` + recepción existen, falta conciliación automática.
